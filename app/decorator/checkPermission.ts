 ...
 * @LastEditors: Roy
 * @LastEditTime: 2022-01-09 17:31:28
 * @Deprecated: 否
 ...
 * @param errnoType 返回的错误类型，来自 GlobalErrorTypes
 * @param options 特殊配置选项，可以自定义 action 以及查询条件，详见上面的 IOptions 选项
 * @return function
 */
export default function checkPermission(modelName: string | ModelMapping, errnoType: GlobalErrorTypes, options?: IOptions) {
    return function (prototype, key: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (...args: any[]) {
            ...
            if (rule && rule.conditions) {
                ...
                const certianRecord = await app.model[mongooseModelName].findOne(query).lean()
